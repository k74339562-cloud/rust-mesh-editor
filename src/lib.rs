...
impl RenderState {
    ...
    fn recreate_surface(&mut self, window: Arc<Window>) {
        ...
        if let Ok(new_surface) = self.instance.create_surface(window.clone()) {
            ...
            self.update_camera();
            log::info!("تمت استعادة سياق شاشة أندرويد بنجاح بعد استئناف التطبيق!");
        }
    }
    ...
    fn build_ui_mesh(&self) -> Vec<UiVertex> {
        ...
        // 1. الشريط العلوي (Top Header Bar)
        ...
        // 3. اللوحة الجانبية اليمنى (Right Panel)
        let right_w = 210.0;
...
impl ApplicationHandler for App {
    // دالة استئناف التطبيق (تعالج فتح التطبيق لأول مرة والعودة من زر Home)
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        if self.window.is_none() {
            ...
            // العودة من زر Home: إعادة ربط Vulkan فوراً
    ...
    fn suspended(&mut self, _event_loop: &ActiveEventLoop) {
        log::info!("تم تعليق التطبيق بواسطة نظام أندرويد (Home button pressed)");
    }
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        ...
        match event {
            ...
            // تتبع حركة المؤشر (تم عكس إشارة dx لتكون الحركة متطابقة 100% مع اليد)
            WindowEvent::CursorMoved { position, .. } => {
                ...
                if state.is_orbiting {
                    state.camera_yaw -= dx * 0.008; // تصحيح اتجاه اليمين واليسار
                ...
                match touch.phase {
                    ...
                    TouchPhase::Moved => {
                        ...
                        if state.active_touches.len() == 1 {
                            let dx = x - state.last_cursor_pos.0;
                            let dy = y - state.last_cursor_pos.1;
                            state.camera_yaw -= dx * 0.008; // تصحيح اتجاه اللمس
                            state.camera_pitch = (state.camera_pitch + dy * 0.008).clamp(-1.5, 1.5);
                            state.update_camera();
                            window.request_redraw();
                            state.last_cursor_pos = (x, y);
            ...
            WindowEvent::RedrawRequested => {
                match state.render() {
                    Ok(_) => {}
                    // استعادة السطح التلقائية عند فقدان الذاكرة أو تغير أبعاد الشاشة
